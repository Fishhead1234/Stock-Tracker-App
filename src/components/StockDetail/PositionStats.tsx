...
export const PositionStats: React.FC<Props> = ({ stock, position, onOpenAddModal }) => {
  ...
  const [whatIfShares, setWhatIfShares] = useState<number>(5);
    ...
    <div className="bg-navy-900/90 border border-navy-800 rounded-2xl p-4 space-y-4 shadow-sm">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-xs font-bold text-white uppercase tracking-wider">Your Position</h4>
        </div>
        {position ? (
          <button
      ...
      {position ? (
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2 bg-navy-950/70 p-3 rounded-xl border border-navy-800/80 font-mono text-xs">
            <div>
              <span className="text-[10px] text-slate-400 block font-sans">Shares Owned</span>
              <strong className="text-white text-sm">{position.shares}</strong>
            </div>
            <div>
              <span className="text-[10px] text-slate-400 block font-sans">Avg Buy Price</span>
              <strong className="text-white text-sm">${position.averageBuyPrice.toFixed(2)}</strong>
            </div>
            <div className="pt-2 border-t border-navy-800/60">
              ...
              <strong className="text-slate-300">${costBasis.toFixed(2)}</strong>
            </div>
            <div className="pt-2 border-t border-navy-800/60">
              <span className="text-[10px] text-slate-400 block font-sans">Unrealized Return</span>
          ...
          {position.notes && (
            <div className="text-xs text-slate-400 bg-navy-950/40 p-2.5 rounded-lg border border-navy-800 italic">
              <span className="font-semibold text-slate-300 not-italic font-sans block text-[10px] uppercase">Your Thesis:</span>
              "{position.notes}"
            </div>
          )}
        </div>
        ...
        <div className="p-3 bg-navy-950/50 rounded-xl border border-navy-800 text-center">
          <p className="text-xs text-slate-400 mb-2">You don't own any shares of {stock.ticker} yet.</p>
          <button
            onClick={onOpenAddModal}
            className="w-full py-2 bg-growth-600 hover:bg-growth-500 text-white font-semibold text-xs rounded-lg transition"
          >
            Log Shares You Bought
          </button>
        </div>
      )}

      {/* Beginner What-If / Dollar-Cost Average Simulator */}
      <div className="pt-3 border-t border-navy-800">
        ...
        <p className="text-[11px] text-slate-400 mb-2.5">
          See how buying more shares at today's price (${stock.price.toFixed(2)}) changes your average cost.
        </p>
        ...
        <div className="bg-navy-950/80 p-3 rounded-xl border border-navy-800 space-y-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-16 bg-navy-900 border border-navy-700 rounded px-2 py-1 text-right font-mono text-white text-xs"
              />
          ...
          <div className="pt-2 border-t border-navy-800/80 flex items-center justify-between text-xs font-mono">
            <span className="text-slate-400 font-sans">New Average Cost:</span>
            <span className="text-growth-400 font-bold text-sm">${newAveragePrice.toFixed(2)}</span>
          </div>
